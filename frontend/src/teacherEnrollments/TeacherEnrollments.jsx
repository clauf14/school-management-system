...
const TeacherEnrollments = () => {
  ...
  const fetchTableData = async (id) => {
    ...
    try {
      ...
      const studentList = enrollmentRes.data.map((enrollment) => ({
        ...enrollment.student, // keep all student fields
        enrollmentId: enrollment.enrollmentId, // also keep enrollmentId
      }))
  ...
  const columns = [
          ...
          cell: (row) => (
            <div className="grade-cell">
              <input
                ...
                onBlur={(e) => handleGradeSubmit(row.studentId, assignment.assignmentId, e.target.value)}
                className="grade-input"
              />
              <button className="history-button" title="View grade history" onClick={() => handleSeeGradeHistories(assignment.assignmentId, row.studentId)}>
                🕓
              </button>
            </div>
          ),
      ...
      : []),
    {
      ...
      selector: (row) => `${row.firstName} ${row.lastName}\n${formatDate(row.createdAt)}`,
      cell: (row) => (
        <button className="remove-button" onClick={() => deleteEnrollment(row.enrollmentId)} title="Remove student">
          ❌
        </button>
      ),
    },
    ...
    <div className="data-table">
      <div className="container">
        <div className="data-table-wrapper">
          ...
          <DataTable columns={columns} data={students} progressPending={loading} pagination paginationPerPage={5} highlightOnHover pointerOnHover responsive />
        </div>
        ...
        <dialog open={isAddStudentDialogOpen}>
          <form onSubmit={handleAddStudentSubmit}>
            ...
            <input type="text" id="studentEmail" name="studentEmail" value={studentEmail} onChange={(e) => setStudentEmail(e.target.value)} required />
            <div>
              <button type="button" onClick={handleCloseDialogs}>
                Cancel
              </button>
        ...
        <dialog open={isAddAssignmentDialogOpen}>
          <form onSubmit={handleAddAssignmentSubmit}>
            ...
            <input type="text" id="assignmentName" name="assignmentName" value={assignmentName} onChange={(e) => setAssignmentName(e.target.value)} required />
            <label htmlFor="assignmentScore">Assignment max score:</label>
            <input type="number" min="1" max="10" step="1" id="assignmentScore" name="assignmentScore" value={assignmentMaxScore} onChange={(e) => setAssignmentMaxScore(e.target.value)} required />
            <div>
              <button type="button" onClick={handleCloseDialogs}>
                Cancel
              </button>
        ...
        <dialog open={isSeeGradeHistoryOpen} className="dialog-container">
          <div>
              ...
              <div className="grade-history-list">
                {gradeHistories.map((history, index) => (
                  <div key={index} className="grade-history-item">
                    <div>
                      <strong className="grade-history-scores">
                        {history.oldScore} → {history.newScore}
                      </strong>
                      <div className="grade-history-date">{formatDate(history.changedAt)}</div>
                    </div>
            ...
            <div className="close-button-container">
              <button onClick={() => setIsSeeGradeHistoryOpen(false)} className="close-button">
                Close
              </button>
            </div>
          </div>
